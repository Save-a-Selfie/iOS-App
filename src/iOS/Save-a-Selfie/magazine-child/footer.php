...
var root = 'http://www.iculture.info/saveaselfie/wp-content/themes/magazine-child/icons/';
var customIcons = {
  0: {
	icon: root + 'defibrillatorSm.jpg'
  },
  1: {
...
// 		console.log('home (' + mg + ', ' + instagramImages.width() + ', ' + jQuery('body').width() + ')');
	...
	jQuery('#footerwrap').css('top', jQuery(window).height() + 'px');
	setTimeout(function(){
		var topV = jQuery('#body').offset().top + jQuery('#body').height() + 20 + (macComputer || villageIdiot ? 2 : 2);
		if (!clickToAdded) {
