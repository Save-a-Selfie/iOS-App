function noFloat() {
	...
	setTimeout(function(){ fm.css({'width': '0'}); mapOff = true; floatingImage.css({'width': '0'}); }, fadeTime);
}
...
// 	console.log(Object.values(images));
	...
	image.onload = function(){
		...
		setTimeout(function(){
			...
			jQuery('#x').css({'opacity':0, 'top':'0px', 'left':'5px', 'color':'white', 'position':'absolute', 'font-size':'20px', 'text-shadow': '2px 2px 2px rgba(150, 150, 150, 0.75)'}).delay(fadeTime).fadeTo(fadeTime, 1).click(noFloat);
		}, fadeTime);
...
function floatIt(id, e, x, y) {
	...
	setTimeout(function(){
		...
		if (floatingOpen == -1) {
			...
			jQuery('#' + id).css({'background-image':'url(http://www.saveaselfie.org/wp/wp-content/uploads/2014/09/loading.gif)', 'background-repeat':'no-repeat', 'background-size':'contain'});
		}
		floatPageX = e.pageX;
